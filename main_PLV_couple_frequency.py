...
class DSEN(nn.Module):
    def __init__(self, num_features=128, num_channels=30, num_segments=9, num_frequency_bands=1):
    ...
    def forward(self, x):
        batch_size = x.size(0)
        # Now we have only one frequency band: x shape: (batch_size, 1, num_channels, num_channels)
        num_channels = x.size(2)

        # Extract the single frequency band (index 0)
        plv_matrix = x[:, 0, :, :]  # Shape: (batch_size, num_channels, num_channels)

        # Extract upper triangle indices
        triu_indices = torch.triu_indices(num_channels, num_channels, offset=1)
        upper_tri = plv_matrix[:, triu_indices[0], triu_indices[1]]  # (batch_size, num_features)

        # upper_tri is now our feature vector for this single band
        x = upper_tri

        # If you need to reshape to (batch_size, num_channels, time_len), ensure divisibility
        plv_feature_size = x.size(1)
        remainder = plv_feature_size % num_channels
        if remainder != 0:
            # Pad to make it divisible
            needed = num_channels - remainder
            pad = torch.zeros(batch_size, needed, dtype=x.dtype, device=x.device)
            x = torch.cat([x, pad], dim=1)
            plv_feature_size = x.size(1)

        self.time_len = plv_feature_size // num_channels
        x = x.view(batch_size, num_channels, self.time_len)  # (batch_size, num_channels, time_len)

        # Pass through your blocks
        x = self.block_1(x)  # (batch_size, num_channels, 100)
        x = self.block_2(x)  # (batch_size, num_channels, 128)

        x = x.view(batch_size * num_channels, -1)  # (batch_size * num_channels, 128)

        # Create edge_index for a single graph
        edge_index = create_fully_connected_edge_index(num_channels).to(x.device)
        ...
        # Create batch tensor
        batch = torch.arange(batch_size, device=x.device).unsqueeze(1).repeat(1, num_channels).view(-1)
        ...
        return out  # (batch_size, 128) embedding
...
def create_pairs_and_triplets(data, labels):
    ...
    # Calculate the number of pairs and triplets based on data size
    num_pairs = min(1000, len(labels) * 10)  # For example, 10 times the number of samples
    num_triplets = min(1000, len(labels) * 5)  # For example, 5 times the number of samples
...
def create_pairs_and_triplets(data, labels):
    ...
    if len(labels_with_enough_samples) < 2:
        # We only have one class with enough samples
        # Can only create positive pairs (no negative pairs or triplets)
        print("Only one class with enough samples. Creating only positive pairs (no negative pairs or triplets).")
        for _ in range(num_pairs):
            label = random.choice(labels_with_enough_samples)
            idx1, idx2 = random.sample(label_to_indices[label], 2)
            pairs.append((data[idx1], data[idx2]))
            pair_labels.append(1)
        # No negative pairs or triplets
        return pairs, pair_labels, triplets

    # If we have at least two classes with enough samples, proceed as normal
    # Generate positive and negative pairs
    for _ in range(num_pairs // 2):
        # Positive pair (same label)
        label = random.choice(labels_with_enough_samples)
...
if __name__ == '__main__':
    ...
    #friend_ids = [23, 25, 27, 35, 37, 43, 47, 49, 51, 57, 59, 67, 69, 73, 77, 79, 81, 83, 85, 105, 107,
                  #131, 137, 143, 153, 159]
    friend_ids = [41,61,63,65,71,87,109,115,121,127,129,133,135,139,141,147,149,151,155,157,163,165,167,175,177,
                       189,199,201,203]
    ...
    LowBand = 8  # original 13 but now 4 frequency so adjusted to 4.
    HighBand = 12  # band upper edge
