...
#friend_ids = [23,25,27,35,37,43,47,49,51,57,59,67,69,73,77,79,81,83,85,105,107,
     #131,137,143,153,159]
friend_ids = [41, 61, 63, 65, 71, 87, 109, 115, 121, 127, 129, 133, 135, 139, 141, 147, 149, 151, 155, 157, 163, 165,
...
def test_model(model, test_loader, device, actual_labels):
    ...
    report = classification_report(
        ...
        all_preds,
        labels=[0, 1],
        target_names=['Stranger', 'Friend(Couple)'],
        zero_division=0
    )
...
if __name__ == '__main__':
    ...
    for i, eeg_data in enumerate(data):
        print(
            ...
            f"Label = {'Friend(Couple)' if labels[i] == 1 else 'Stranger'}"
        )
    ...
    LowBand = 8  # Beta band lower edge
    HighBand = 12  # Gamma band upper edge
    ...
    for idx, pred in enumerate(all_preds):
        ...
        print(
            f'Pair {idx + 1}: Predicted = {"Friend(Couple)" if pred == 1 else "Stranger"}, '
            f'Actual = {"Friend(Couple)" if actual_label == 1 else "Stranger"}'
        )
